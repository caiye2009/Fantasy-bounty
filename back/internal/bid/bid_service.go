...
type Service interface {
	...
	ListBidsByUsername(ctx context.Context, username string, status string, page, pageSize int) ([]Bid, int64, error)
}
...
// CreateBid 创建新竞标
func (s *service) CreateBid(ctx context.Context, username string, req *CreateBidRequest) (*Bid, error) {
	bid := &Bid{
		ID:       uuid.New().String(),
...
// ListBidsByUsername 根据用户名获取竞标列表
func (s *service) ListBidsByUsername(ctx context.Context, username string, status string, page, pageSize int) ([]Bid, int64, error) {
	if page < 1 {
		page = 1
	}
	if pageSize < 1 || pageSize > 100 {
	...
	offset := (page - 1) * pageSize
	return s.repo.ListByUsername(ctx, username, status, offset, pageSize)
}
